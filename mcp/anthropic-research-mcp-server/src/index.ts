...
const GetPublicationInputSchema = z
  .object({
    url: z
      .string()
      .url()
      .describe("Publication URL (usually from anthropic_list_publications)."),
    response_format: ResponseFormatSchema.describe("Output format: markdown or json."),
  })
  .strict();
...
function absolutize(href: string): string {
  ...
  if (href.startsWith("/")) return `https://www.anthropic.com${href}`;
...
function parsePublicationsFromResearchPage(html: string): PublicationListItem[] {
  const $ = cheerio.load(html);
  const items: PublicationListItem[] = [];

  // Heuristic: collect internal links under /research/ that look like publication pages.
  ...
  // Deduplicate near-empty titles and sort by URL stability (best-effort).
  return items
...
async function fetchPublicationDetail(url: string): Promise<PublicationDetail> {
  ...
  // Best-effort date/category from nearby text nodes (page-dependent).
  const maybeDate = normalizeWhitespace(
    $("main time").first().attr("datetime") ?? $("main time").first().text() ?? "",
  );
...
function renderListMarkdown(result: {
  ...
  const lines: string[] = [];
  lines.push("## Anthropic Research publications");
  lines.push(
...
async function main() {
  const server = new McpServer({ name: "anthropic-research-mcp-server", version: "0.1.0" });

  server.registerTool(
    "anthropic_list_publications",
    {
      title: "List Anthropic Research publications",
      description:
        "List publication links from Anthropic Research. Supports simple substring filtering by title and offset/limit pagination.",
    ...
    async ({ limit, offset, query, response_format }) => {
      try {
        ...
        let items = parsePublicationsFromResearchPage(html);
        if (query) {
  ...
  server.registerTool(
    "anthropic_get_publication",
    {
      title: "Get Anthropic publication details",
      description:
        "Fetch a single Anthropic Research publication page and extract title, date (if present), headings, and a short excerpt.",
